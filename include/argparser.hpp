...
inline bool starts_with(const char *prefix, const char *arg) { return memcmp(prefix, arg, strlen(prefix)) == 0; }

inline bool starts_with(char prefix, const char *arg) { return arg[0] == prefix; }
  ...
  if (long_name != nullptr) {
    ...
    while (long_name[i] != '\0') {
      if (isspace(long_name[i])) {
        std::cerr << "Error: ArgParser -> option names can not have whitespaces: '" << long_name << '\'' << std::endl;
...
struct Option {
  ...
  Option(bool rv, const char *help, const char *lname, char sname, func f)
    : requires_value(rv), help_message(help), long_name(lname), short_name(sname), accept_value(f)
  {}
...
struct Argument {
  ...
  Argument(const char *help, const char *name, func f) : help_message(help), name(name), accept_value(f) {}
...
public:
  ...
  ArgParser(const char *program_name) : m_options({}), m_positional_arguments({}), m_program_name(program_name)
  {
    // Add the help option implicitely.
    m_options.push_back(Option(false, "Print this help message", "help", '\0', [this](const char *) {
      print_help();
...
{
  verify_names(long_name, short_name);
  Option opt(true, help_msg, long_name, short_name, [&value](const char *arg) {
    try {
      value = std::stoi(arg);
...
inline void ArgParser::add_option(const char *&value, const char *help_msg, const char *long_name, char short_name)
{
  verify_names(long_name, short_name);
...
{
  Argument arg(help_msg, name, [&value](const char *parg) {
    try {
      value = std::stoi(parg);
    } catch (std::invalid_argument &e) {
      std::cerr << "Error: wrong argument type, integer expected." << std::endl;
...
inline void ArgParser::parse(const int argc, char **argv)
{
  for (int i = 1; i < argc; ++i) {
    ...
    if (starts_with("--", argv[i])) {
      ...
      if (m_curr_arg < m_positional_arguments.size()) { m_positional_arguments.at(m_curr_arg++).accept_value(argv[i]); }
    }
  }
}
...
#define FIELD_WIDTH 20
  ...
  for (auto &arg : m_positional_arguments) std::cout << std::setw(FIELD_WIDTH) << arg.name << arg.help_message << '\n';
